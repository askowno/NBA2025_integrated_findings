...
#| warning: false
#| fig-label: rle-plot
#| fig-cap: "Figure 1. (A) Percentage of ecosystem types in each threat status category in each realm; (B) the percentage extent of each ecosystem in each threat status category (note for that length is used for rivers, while area is used for all other realms). In addition to the realms, the plot also shows results for the coastal zone, made up of estuaries and selected marine and terrestrial types, and the subantarctic territory of South Africa."
#| fig-align: "center"
#| fig-show: "hold"
...
# using patchwork to combine (this makes auto use of output impossible hence the ggsave and then additionof the image)
...
# Save to file incase this doesnt work
